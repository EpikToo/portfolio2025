...
const SectionTitle = ({ children, color = "bg-blue-600" }) => (
    <div className={`${color} text-white px-2 py-1 mb-2 rounded-sm`}>
        <h2 className="text-base md:text-lg font-bold truncate">{children}</h2>
    </div>
);
...
const TimelineItem = ({ content, isCurrent }) => {
    ...
    return (
        <div className="mb-2 md:mb-4 pb-2 border-b border-gray-200 last:border-0 hover:bg-gray-50 transition-colors p-1 md:p-2">
            <div className="flex flex-col sm:flex-row">
                ...
                <div className="sm:w-2/3 text-xs md:text-sm break-words">
                    ...
                    {isCurrent && (
                        <span className="ml-2 bg-green-100 text-green-800 text-xs px-1 md:px-2 py-0.5 rounded shadow-win98-btn whitespace-nowrap">
                            Current
                        </span>
                    )}
                </div>
